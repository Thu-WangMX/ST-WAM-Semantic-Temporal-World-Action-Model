...
def get_model(usr_args: Dict[str, Any]):
    ...
    vlm_model_name_or_path = usr_args.get("vlm_model_name_or_path")
    if not _is_none_like(vlm_model_name_or_path):
        if cfg.model.get("semantic_history_config") is None:
            raise ValueError(
                "`vlm_model_name_or_path` was provided, but the model has no "
                "`semantic_history_config`."
            )
        OmegaConf.update(
            cfg,
            "model.semantic_history_config.vlm_model_name_or_path",
            str(vlm_model_name_or_path),
        )
        OmegaConf.update(
            cfg,
            "model.semantic_history_config.allow_vlm_path_relocation",
            _parse_bool(usr_args.get("allow_vlm_path_relocation", False)),
            force_add=True,
        )
