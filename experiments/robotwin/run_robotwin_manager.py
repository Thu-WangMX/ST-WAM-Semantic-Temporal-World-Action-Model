...
def _resolve_tasks(task_name_cfg: Any, task_names_cfg: Any) -> list[str]:
    has_task_name = task_name_cfg is not None and str(task_name_cfg).strip() != ""
    has_task_names = task_names_cfg is not None
    if has_task_name and has_task_names:
        raise ValueError("Set only one of `EVALUATION.task_name` and `EVALUATION.task_names`.")

    all_tasks = _load_all_tasks()
    if has_task_name:
        requested = [str(task_name_cfg).strip()]
    elif has_task_names:
        if isinstance(task_names_cfg, str):
            raise TypeError("`EVALUATION.task_names` must be a list, not a string.")
        requested = [str(task).strip() for task in task_names_cfg]
        if len(requested) == 0 or any(task == "" for task in requested):
            raise ValueError("`EVALUATION.task_names` must contain at least one non-empty task.")
    else:
        return all_tasks

    duplicates = sorted({task for task in requested if requested.count(task) > 1})
    if duplicates:
        raise ValueError(f"Duplicate tasks in evaluation selection: {duplicates}")
...
def main(cfg: DictConfig):
    ...
    tasks = _resolve_tasks(cfg.EVALUATION.task_name, cfg.EVALUATION.task_names)
